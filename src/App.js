...
import useGeoLocation from "./hooks/useLoaction";
...
function App({ auth }) {
  ...
  const latLong = reactLocalStorage.get("User_Id", false);
  // let parseData = JSON.parse(latLong);

  console.log("nsghdfjhghds", Livelocation);
  onMessageListener()
    .then((payload) => {
      setShow(true);
      setNotification({
      ...
      if (payload?.data?.userid !== latLong) {
        console.log("payload", payload?.data?.userid);
        toast.success(payload.notification.body);
      }
    })
    .catch((err) => console.log("failed: ", err));

  console.log("dhvjdsfsfsbj", notification);
  ...
  useEffect(() => {
    ...
    reactLocalStorage.set("latLong", JSON.stringify(Livelocation));
  }, [Livelocation]);
