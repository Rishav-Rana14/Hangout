  ...
  const SendOtp = async() => {
   ...
   await axios
      ...
      .then((Response) => {
        ...
        reactLocalStorage.set("token", Response?.data?.tokens?.access?.token);
        reactLocalStorage.set("User_Id", Response?.data?.user?.id);
        let Permission = JSON.stringify(Response?.data?.permission);
        console.log("Permission_Data",Response);
            ...
            <div className="right_part login-part">
              ...
              <div className="sign_up_ " >
                ...
                <div className="input_filed text-center mb-3  ">
                  ...
                  <input type="text" value={email} placeholder="Phone No." onChange={(e) => setEmail(e.target.value)} />
                </div>
